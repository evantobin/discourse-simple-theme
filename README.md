...
- **Brand gradient moments** — the signature orange → pink → violet gradient (normally reserved for brand/marketing contexts, not product chrome) shows up as: a 3px accent strip along the top of the header, a gradient headline on the homepage hero banner, gradient-filled primary buttons with a sweep-in hover animation, accent-colored tag chips (cycled across the six brand accent colors, since plain tags have no inherent color the way categories do), and a gradient-text hover on links inside post content.
